...
data "null_data_source" "userdata_linux" {
  count = var.operatingsystem=="linux"?var.instance_count:0
   inputs    = {
    ...
    echo '{"elevated_groups": ${jsonencode(var.securitygroup_administrators)}}' >/etc/puppetlabs/facter/facts.d/elevated_groups.json
...
data "null_data_source" "userdata_windows" {
  ...
  inputs = {
    ...
    <script>
     ...
     echo {"elevated_groups":${jsonencode(var.securitygroup_administrators)}} > C:\ProgramData\PuppetLabs\facter\facts.d\elevated_group.json
